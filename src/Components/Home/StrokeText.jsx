...
const StrokeText = ({
  ...
  useLayoutEffect(() => {
    ...
    const isGeneric = !primaryFont || ['sans-serif', 'serif', 'monospace', 'inherit', 'initial'].includes(primaryFont);

    const isFontReady = () => {
      if (typeof document === 'undefined' || !document.fonts?.check || isGeneric) return true;
      return document.fonts.check(`1em "${primaryFont}"`);
    };

    const measure = () => {
      if (cancelled || !strokeTextRef.current) return;
      if (!allowFallback && !isFontReady()) return;
      let bbox;
      try {
    ...
    if (typeof document !== 'undefined' && document.fonts) {
      document.fonts.load(`${fontWeight} ${fontSize}px "${primaryFont}"`).then(measure).catch(measure);
  ...
  const fallbackWidth = Math.max(characters.length * fontSize * 0.4, 100);
  const viewBox = box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-fontSize} ${fallbackWidth} ${fontSize}`;
